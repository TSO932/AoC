...
let vaporise(spaceMap:string,stationLocation) =
    ...
    let matrixOfAngles = matrix |> Array2D.mapi (fun v h c -> (c = '#' && not (v = snd stationLocation && h = fst stationLocation), System.Math.Atan2(float (h - fst stationLocation),float (v - snd stationLocation))))
    ...
    let nextAngleOfAttack = matrixOfAngles |> Seq.cast<bool*double> |> Seq.filter (fun x -> fst x) |> Seq.maxBy snd
    printfn ">>>>>>>>>>>>>>>>>>>>>> %A" nextAngleOfAttack
